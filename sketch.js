...
function preload(){
   ...
   fontBold = loadFont("assets/facon.ttf")
   fontSL = loadFont("assets/texgyreadventor-bold.otf")
}
function setup(){
...
function draw(){
  if(gameState === START){
    ...
    textFont(fontBold)
    textSize(50)
    text("Road Rash",550,200)

    fill('black')
    textFont(fontSL)
    text("Press Tab to Start",500,400)
  ...
  if(keyDown('space') && runner.y > 512.5){
    runner.velocityY = -20
    console.log("Working")
  }
